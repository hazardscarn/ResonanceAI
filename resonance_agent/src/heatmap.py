...
def get_complete_heatmap_analysis(
    ...
    limit: int = 50
) -> pd.DataFrame:  # ← Changed return type annotation to be explicit
    """
    Core heatmap function - resolves names to IDs and returns DataFrame
    ...
    Args:
        ...
        audience_ids: Optional list of audience IDs for context
        audience_weight: Optional weight for audience influence (0-1)
    ...
    Returns:
        DataFrame with heatmap data points and strategy columns
    ...
    empty_df = pd.DataFrame(columns=[
        'latitude', 'longitude', 'geohash', 'affinity', 'popularity', 
        'affinity_rank', 'hotspot_score', 'segment', 'strategy'
    ])
    ...
    try:
        ...
        signals = None
        if age or gender or audience_ids:
            demographics = {}
            if age:
        ...
        try:
            ...
            # Return empty DataFrame instead of result dictionary
            return empty_df
        
        # Step 5: Process heatmap results
        if heatmap_result.get("success"):
            ...
            if data_points:
                ...
                # No data points found, return empty DataFrame
                logger.warning(f"No heatmap data points found for {location_query}")
                return empty_df
